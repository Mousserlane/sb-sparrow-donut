...
class DataAnnotation:
    class Model:
        ...
        subgroup_file = "docs/subgroups.json"
        ...
        subgroups = ["", "subitem_row1", "subitem_row2", "subitem_row3"]
    ...
    def view(self, model, ui_width, device_type, device_width):
        ...
        with open(model.subgroup_file, "r") as f:
            subgroup_json = json.load(f)

        subgroups_list = subgroup_json["subgroups"]
        subgroups = [""]
        ...
        with st.sidebar:
            ...
            annotation_selection = placeholder_upload.selectbox(
        ...
        with st.container():
            ...
            if number_of_columns > 1:
                ...
                with col2:
                    tab = st.radio(
                        "Select",
                        ["Mapping", "Grouping", "Ordering"],
                        horizontal=True,
                        label_visibility="collapsed",
                    )
                    if tab == "Mapping":
                        self.render_form(
                            model, result_rects, data_processor, annotation_selection
                        )
                    elif tab == "Grouping":
    ...
    def render_doc(
        self, model, docImg, saved_state, mode, canvas_width, doc_height, doc_width
    ):
        with st.container():
            height = 1296
            width = 864
    ...
    def render_form_view(self, words, labels, result_rects, data_processor):
        ...
        green_light = "#abf7b1"
        css = {".row-selected": {"background-color": f"{green_light} !important"}}

        response = agstyler.draw_grid(
            df, formatter=formatter, fit_columns=True, grid_options=go, css=css
        )
    ...
    def canvas_available_width(self, ui_width, doc_width, device_type, device_width):
        ...
        if doc_width_pct < 45:
            canvas_width_pct = CANVAS_WIDTH_SM  # 37
        elif doc_width_pct < 55:
            canvas_width_pct = CANVAS_WIDTH_MD
        else:
            canvas_width_pct = CANVAS_WIDTH_LG

        if (
            ui_width > 700
            and canvas_width_pct == CANVAS_WIDTH_SM
            and device_type == "desktop"
        ):
            return math.floor(canvas_width_pct * ui_width / 100), 4
        elif (
            ui_width > 700
            and canvas_width_pct == CANVAS_WIDTH_MD
            and device_type == "desktop"
        ):
            return math.floor(canvas_width_pct * ui_width / 100), 7
        elif (
            ui_width > 700
            and canvas_width_pct == CANVAS_WIDTH_LG
            and device_type == "desktop"
        ):
            return math.floor(canvas_width_pct * ui_width / 100), 1
        else:
    ...
    def fetch_annotations(self, rects_file):
        ...
        if rects_file not in st.session_state:
            if not os.path.exists(rects_file):
                with open(rects_file, "w") as f:
                    saved_state = json.dump({}, f)
            else:
        ...
        if uploaded_file is not None:
            ...
            annotations_json = {
                "meta": {
                    ...
                    "image_size": {"width": img_file.width, "height": img_file.height},
                },
                "words": [],
            }
    ...
    def order_annotations(self, model, labels, groups, subgroups, result_rects):
        if result_rects is not None:
            ...
            for i, rect in enumerate(words):
                if rect["label"] != "":
                    ...
                    # )
                    if ":" in rect["label"]:
                        parts = rect["label"].split(":", 2)
                    else:
                        parts = (None, rect["label"])
                    if parts:
                        group = parts[0]
                        label = parts[-1]
                        subgroup = parts[1] if len(parts) == 3 else None
                        data.append(
                            {
                                "id": i,
                                "value": rect["value"],
                ...
                "group": (
                    "Group",
                    {
                        **PINLEFT,
                        ...
                        "editable": True,
                        "cellEditor": "agSelectCellEditor",
                        "cellEditorParams": {"values": groups},
                    },
                ),
                "subgroup": (
                    "Sub Group",
                    {
                        **PINLEFT,
                        ...
                        "editable": True,
                        "cellEditor": "agSelectCellEditor",
                        "cellEditorParams": {"values": subgroups},
                    },
                ),
            }
            ...
            go = {
                "rowClassRules": {
                    "row-selected": "data.id === "
                    + str(result_rects.current_rect_index)
                }
            }
            ...
            props = {
                "buttons": {
                    "up": {"disabled": False, "rendered": ""},
                    "down": {"disabled": False, "rendered": ""},
                    "save": {
                        "disabled": False,
                        "rendered": "",
                        # 'rendered': 'none',
            ...
            if len(rows) == 0 and result_rects.current_rect_index > -1:
                for i, row in enumerate(data):
                    if row["id"] == result_rects.current_rect_index:
                        rows = [
                            {
                                "_selectedRowNodeInfo": {"nodeRowIndex": i},
                                "id": row["id"],
                            }
                        ]
                        break
                ...
                for elem in data:
                    if elem[3] != "None":
                        ...
                        if subgroup and subgroup != "None":
                            words[idx]["label"] = f"{group}:{subgroup}:{elem[2]}"
                        else:
                        ...
                        if word["label"] != "":
                            if ":" in word["label"]:
                                ...
                                if "row" not in group:
                                    ...
                                    if "subitem" in label:
                                        subgroup, sublabel = label.split(":", 1)
                                        if "subitems" not in row_keys[group]:
                                            row_keys[group]["subitems"] = []

                                        if subgroup not in subrow_keys:
                                            subrow_keys[subgroup] = {}

                                        subrow_keys[subgroup][sublabel] = word["value"]

                                        if (
                                            subrow_keys[subgroup]
                                            not in row_keys[group]["subitems"]
                                        ):
                                            row_keys[group]["subitems"].append(
                                                subrow_keys[subgroup]
                                            )
                                    ...
                                    keys_to_delete = [
                                        key
                                        for key in row_keys[group].keys()
                                        if "subitem_row" in key
                                    ]

                                    for key in keys_to_delete:
                                        if "subitem_row" in key:
                                            del row_keys[group][key]
                            else:
